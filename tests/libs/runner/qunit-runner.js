...
(function(window) {
    ...
    runner.prototype.runPage = function() {
        ...
        test.header = $('<p class="runner-test-page-header"><a href="' + test.page + '">' + test.title + "</a> <span class=\"status\"></span></p>").addClass("passed")[0];
    ...
    runner.prototype.pageProgress = function(frame, failures, total, testName, isDone) {
        ...
        $.grep(this.tests, function(test, index) {
            if (test.frame === frame) {
				...
				if (failures > 0) {
					$(test.header).next(".runner-test-page-frame").show(100);
				}
            }
        });
    }
